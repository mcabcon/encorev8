...
{
	/#
	debug_replay("File: _callbackglobal.gsc. Function: Callback_PlayerConnect()\n");
	#/

	thread first_player_connect(); 

	/#
	debug_replay("File: _callbackglobal.gsc. Function: Callback_PlayerConnect() - START WAIT begin and waittillframeend\n" ); 
	#/
	...
	/#
	debug_replay( "File: _callbackglobal.gsc. Function: Callback_PlayerConnect() - STOP WAIT begin and waittillframeend\n" ); 
	#/
...
#/
	...
	/#
	debug_replay( "File: _callbackglobal.gsc. Function: Callback_PlayerConnect() - COMPLETE\n" ); 
	#/
}
...
Callback_ActorDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	else if (!is_true(self.a.doingRagdollDeath))
	{
		...
		if( GetDvar( #"zombiemode") != "1" )
		{
			...
			{
				// AI revive feature - modifies the fatal damage if needed to kick in revive script
...
{
/#
	debug_replay( "File: _callbackglobal.gsc. Function: first_player_connect()\n" ); 
#/
...
/#
	debug_replay( "File: _callbackglobal.gsc. Function: first_player_connect() - COMPLETE\n" ); 
#/
}
