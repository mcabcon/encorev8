...
freezegun_play_all_torso_damage_fx( localclientnum )
{
	...
	if ( isDefined( level.use_freezegun_features ) && level.use_freezegun_features == true )
	{
		is_gibbed = self clientscripts\_zombiemode::has_gibbed_piece( level._ZOMBIE_GIB_PIECE_INDEX_LEFT_LEG ) || self clientscripts\_zombiemode::has_gibbed_piece( level._ZOMBIE_GIB_PIECE_INDEX_RIGHT_LEG );
		if ( is_gibbed )
		{
